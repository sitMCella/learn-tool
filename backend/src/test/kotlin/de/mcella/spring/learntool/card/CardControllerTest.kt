...
class CardControllerTest {
    ...
    @Test
    @WithMockUser
    fun `given a Workspace name and a CardContent, when sending a POST REST request to the cards endpoint and the cardService create method throws UserNotAuthorizedException, then a UNAUTHORIZED http status response is returned`() {
        val workspace = WorkspaceRequest("workspaceTest")
        val cardContent = CardContent("request", "response")
        val contentBody = objectMapper.writeValueAsString(cardContent)
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
    ...
    @Test
    @WithMockUser
    fun `given a Workspace name, a Card Id, and a CardContent, when sending a PUT REST request to the cards endpoint and the cardService update method throws UserNotAuthorizedException, then a UNAUTHORIZED http status response is returned`() {
        val workspace = WorkspaceRequest("workspaceTest")
        val cardId = CardId("9e493dc0-ef75-403f-b5d6-ed510634f8a6")
        val cardContent = CardContent("request", "response")
        val contentBody = objectMapper.writeValueAsString(cardContent)
    ...
    fun `given a Workspace name, a Card Id, and a CardContent, when sending a PUT REST request to the cards endpoint and the cardService update method throws InvalidWorkspaceNameException, then an UNPROCESSABLE_ENTITY http status response is returned`() {
        ...
        val contentBody = objectMapper.writeValueAsString(cardContent)
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
        Mockito.`when`(cardService.update(cardId, workspace, cardContent, user)).thenThrow(InvalidWorkspaceNameException("The provided workspaceName does not match with the card workspace"))
    ...
    @Test
    @WithMockUser
    fun `given a Workspace name and a Card Id, when sending a DELETE REST request to the cards endpoint and the cardService update method throws UserNotAuthorizedException, then an UNAUTHORIZED http status response is returned`() {
        val workspace = WorkspaceRequest("workspaceTest")
        val cardId = CardId("9e493dc0-ef75-403f-b5d6-ed510634f8a6")
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
    ...
    fun `given a Workspace name and a Card Id, when sending a DELETE REST request to the cards endpoint and the cardService update method throws InvalidWorkspaceNameException, then an UNPROCESSABLE_ENTITY http status response is returned`() {
        ...
        val cardId = CardId("9e493dc0-ef75-403f-b5d6-ed510634f8a6")
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
        Mockito.`when`(cardService.delete(cardId, workspace, user)).thenThrow(InvalidWorkspaceNameException("The provided workspaceName does not match with the card workspace"))
    ...
    @Test
    @WithMockUser
    fun `given a Workspace name, when sending a GET REST request to the cards endpoint and the CardService findByWorkspace mothod throws WorkspaceNotExistsException exception, then an INTERNAL_SERVER_ERROR http status response is returned`() {
        val workspace = WorkspaceRequest("workspaceTest")
        val cardPagination = CardPagination(0, 20)
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
    ...
    @Test
    @WithMockUser
    fun `given a Workspace name, when sending a GET REST request to the cards endpoint and the cardService findByWorkspace method throws UserNotAuthorizedException, then an UNAUTHORIZED http status response is returned`() {
        val workspace = WorkspaceRequest("workspaceTest")
        val cardPagination = CardPagination(0, 20)
        val user = UserPrincipal(1L, "test@google.com", "password", Collections.singletonList(SimpleGrantedAuthority("ROLE_USER")), emptyMap())
